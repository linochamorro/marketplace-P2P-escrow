 ...
 * fila ADMIN: la limpieza de {@code @BeforeEach} (TRUNCATE de tablas de catálogo/transacciones
 * más borrado selectivo de usuarios) conserva al único administrador provisionado por el
 * contexto de test (seed V6 con {@code ADMIN_EMAIL}/{@code ADMIN_PASSWORD_HASH}, cuyo hash
 * corresponde a la contraseña plana compartida {@code passwordRaw}) y el escenario que necesita
 * iniciar sesión tras el gate {@code hasRole("ADMIN")} de
 * {@code GET /publicaciones?estado=PENDIENTE_REVISION} lo hace exclusivamente con esa identidad
 * única, resuelta mediante {@link #obtenerAdminUnico()}. Motivo:
...
class ListadoPublicacionesControllerIntegrationTests {
     ...
     * El TRUNCATE ya no incluye {@code usuarios}: el ADMIN único del seed V6 sobrevive a la
     * limpieza y los usuarios auxiliares se eliminan selectivamente (política PHA12TSK06).
     */
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext).apply(springSecurity()).build();
        jdbcTemplate.execute("TRUNCATE TABLE transacciones, publicaciones, subcategorias, categorias CASCADE");
    ...
    /**
     * Persiste un usuario auxiliar con contraseña BCrypt apta para el login real del test
     * (política PHA12TSK06: en esta clase solo se invoca con {@link Rol#USUARIO}; el ADMIN único
     * nunca nace aquí).
     *
